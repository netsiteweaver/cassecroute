...
function init()
{
	//includeJs('/www/assets/js/messages.js');
}
...
function Filter(tableID, text_to_filter,where_to_search) {
    if(text_to_filter.length == 0){
      $('table#'+tableID+' tbody tr').show();
    }else{
      $('table#'+tableID+' tbody tr').hide();
    }
    $("table#" + tableID + " tbody tr").each(function (i,j) {
        var found = false;
        $(this).find(where_to_search).each(function () {
...
//alias for unhide
function show(element){unhide(element)}